...
import { ArrowUpRight, ArrowDownRight, Eye, EyeOff, Sparkles, TrendingUp } from 'lucide-react';
...
const DashboardScreen = ({ onNavigate, movements = [], userProfile, triggerToast }) => {
  ...
  useEffect(() => {
    if (movements.length > 0 && insight.includes("Aún no")) {
  ...
  const handleGenerateInsight = async () => {
    ...
    try {
      ...
      setInsight(result.respuesta || "Hubo un error analizando tus datos. Intentá más tarde.");
    } catch (err) {
      triggerToast("Error con la IA", "error");
    } finally {
    ...
    <div className="min-h-screen bg-[var(--bg-base)] theme-transition pb-32 animate-in fade-in duration-500">
      ...
      <main className="px-6 space-y-6 mt-2">
        ...
        <div onClick={() => onNavigate('pro')} className="bg-gradient-to-r from-[#FFF8E7] to-[#FFF2D6] dark:from-[#3B2F1D] dark:to-[#221A0F] border border-[#FFCE45]/40 rounded-[32px] p-6 flex items-center justify-between gap-4 shadow-[0_4px_20px_rgba(255,206,69,0.1)] relative overflow-hidden group cursor-pointer hover:shadow-[0_8px_30px_rgba(255,206,69,0.2)] transition-all">
          ...
          <button className="bg-[#FFCE45] text-[#221F26] px-5 py-3 rounded-[14px] text-xs font-black uppercase tracking-wider shadow-md group-hover:scale-105 group-active:scale-[0.95] transition-transform relative z-10">Ser PRO</button>
        </div>
